...
export class OrderController {
  ...
  async getSpecificOrder(req, res, next) {
    try {
      const orders = await orderService.getSpecificOrderById(req.query.id);
      res.status(200).send(orders);
    } catch (error) {
      next(error);
    }
  ...
  async getSpecificOrderByWarehouse(req, res, next) {
    try {
      const orders = await orderService.getSpecificOrderByWarehouseId(
