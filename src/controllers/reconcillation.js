...
export class ReconcileController {
    async createReconcile(req, res, next) {
        try {
            const reconcile = await reconcillationService.postReconcillation(req.body);
    ...
    async getWHMreconcillationn(req, res, next) {
        try {
            const reconcileData = await reconcillationService.getWHMwarehouse(req.tokenData, req.query);
