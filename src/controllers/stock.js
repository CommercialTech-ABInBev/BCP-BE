...
export class StockController {
  ...
  async approveCheckOut(req, res, next) {
    try {
      const { id } = req.query;
      const updatedCheckOut = await stockService.approveCheckOuts(
        req.tokenData,
        id
      );
  ...
  async getApprovedStocks(req, res, next) {
    try {
      const approvedStocks = await stockService.getApprovedStocks(req.query);
  ...
  async getAllCheckIns(req, res, next) {
    try {
      const stocks = await stockService.getCheckIns(req.query, req.tokenData);
  ...
  async checkOut(req, res, next) {
    try {
      const { id } = req.query;
      const checkOutData = await stockService.checkOut(
        id,
        req.body,
        req.tokenData
      );
  ...
  async getCheckOut(req, res, next) {
    try {
      const checkOutData = await stockService.getCheckOuts(
        req.query,
        req.tokenData
      );
  ...
  async rejectCheckOut(req, res, next) {
    try {
      ...
      const rejectedCheckOut = await stockService.rejectCheckOuts(
        req.tokenData,
        id
      );
  ...
  async rejectCheckIn(req, res, next) {
    try {
      ...
      const rejectedCheckIn = await stockService.rejectCheckIns(
        req.tokenData,
        id
      );
  ...
  async stockAdjustment(req, res, next) {
    try {
      ...
      const adjustedStock = await stockService.adjustedStock(
        id,
        req.body,
        req.tokenData
      );
  ...
  async approveStockAdjustment(req, res, next) {
    try {
      ...
      const adjustedStock = await stockService.approveAdjustment(
        id,
        req.body,
        req.tokenData
      );
  ...
  async dashBoard(req, res, next) {
    try {
      const dashboardData = await stockService.adminDashboard(req.query);
